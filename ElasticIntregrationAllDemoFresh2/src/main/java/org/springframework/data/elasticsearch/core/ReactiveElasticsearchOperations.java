...
/**
 * Interface that specifies a basic set of Elasticsearch operations executed in
 * a reactive way.
 * <p>
 * Implemented by {@link ReactiveElasticsearchTemplate}. Not often used but a
 * useful option for extensibility and testability (as it can be easily mocked,
 * stubbed, or be the target of a JDK proxy). Command execution using
 * {@link ReactiveElasticsearchOperations} is deferred until a
 * {@link org.reactivestreams.Subscriber} subscribes to the {@link Publisher}.
 *
...
public interface ReactiveElasticsearchOperations {
    ...
    /**
     * Execute within a {@link ClientCallback} managing resources and
     * translating errors.
     *
    ...
    /**
     * Index the given entity, once available, extracting index and type from
     * entity metadata.
     *
    ...
    /**
     * Index the entity, once available, in the given {@literal index}. If the
     * index is {@literal null} or empty the index name provided via entity
     * metadata is used.
     *
     * @param entityPublisher must not be {@literal null}.
    ...
    /**
     * Index the entity in the given {@literal index}. If the index is
     * {@literal null} or empty the index name provided via entity metadata is
     * used.
     *
     * @param entity must not be {@literal null}.
     * @param index the name of the target index. Can be {@literal null}.
    ...
    /**
     * Index the entity, once available, under the given {@literal type} in the
     * given {@literal index}. If the {@literal index} is {@literal null} or
     * empty the index name provided via entity metadata is used. Same for the
     * {@literal type}.
     *
     * @param entityPublisher must not be {@literal null}.
     * @param index the name of the target index. Can be {@literal null}.
     * @param type the name of the type within the index. Can be
     * {@literal null}.
     * @param <T>
     * @return a {@link Mono} emitting the saved entity.
     */
    ...
    /**
     * Index the entity under the given {@literal type} in the given
     * {@literal index}. If the {@literal index} is {@literal null} or empty the
     * index name provided via entity metadata is used. Same for the
     * {@literal type}.
     *
     * @param entity must not be {@literal null}.
     * @param index the name of the target index. Can be {@literal null}.
    ...
    /**
     * Find the document with the given {@literal id} mapped onto the given
     * {@literal entityType}.
     *
     ...
     * @param id the {@literal _id} of the document to fetch.
     * @param entityType the domain type used for mapping the document.
     * @param index the name of the target index. Overwrites document metadata
     * from {@literal entityType} if not {@literal null}.
     * @param <T>
     * @return {@link Mono#empty()} if not found.
     */
     ...
     * @param index the name of the target index. Overwrites document metadata
     * from {@literal entityType} if not {@literal null}.
     * @param type the name of the target type. Overwrites document metadata
     * from {@literal entityType} if not {@literal null}.
     * @param <T>
     * @return the {@link Mono} emitting the entity or signalling completion if
     * none found.
     */
     ...
     * @return a {@link Mono} emitting {@literal true} if a matching document
     ...
     * <br /> {@link Pageable#isUnpaged() Unpaged} queries may overrule
     * elasticsearch server defaults for page size by either delegating to the
     * scroll API or using a max {@link org.elasticsearch.search.builder.SearchSourceBuilder#size(int)
     * size}.
     *
     ...
     *
     * @param query must not be {@literal null}.
     * @param entityType The entity type for mapping the query. Must not be
     * {@literal null}.
     * @param returnType The mapping target type. Must not be {@literal null}.
     * Th
     * @param <T>
     * @return a {@link Flux} emitting matching entities one by one.
     */
     ...
     *
     * @param entity must not be {@literal null}.
     * @return a {@link Mono} emitting the {@literal id} of the removed
     * document.
     */
    ...
    /**
     * Delete the entity with given {@literal id} extracting index and type from
     * entity metadata.
     *
     * @param id must not be {@literal null}.
     * @param entityType must not be {@literal null}.
    ...
    /**
     * Delete the documents matching the given {@link Query} extracting index
     * and type from entity metadata.
     *
     * @param query must not be {@literal null}.
     * @param entityType must not be {@literal null}.
    ...
    /**
     * Callback interface to be used with {@link #execute(ClientCallback)} for
     * operating directly on {@link ReactiveElasticsearchClient}.
     *
     * @param <T>
     * @author Christoph Strobl
     * @since 3.2
